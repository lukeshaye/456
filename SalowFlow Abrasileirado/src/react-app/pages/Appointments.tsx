...
// CORREÇÃO: Importa o novo schema específico para o formulário
import { AppointmentFormSchema } from '../../shared/types';
import { useToastHelpers } from '../contexts/ToastContext';
import ConfirmationModal from '../components/ConfirmationModal';

// --- Configuração e Tipos ---
// Força a configuração do locale brasileiro para o moment.js
moment.locale('pt-br');
...
interface AppointmentFormData {
  client_id: number;
  // client_name não está aqui pois não é um campo do formulário
  service: string;
  price: number;
  professional: string;
...
// CORREÇÃO UX: Valores padrão com `undefined` para permitir que placeholders apareçam.
...
export default function Appointments() {
  ...
  const {
    ...
    // CORREÇÃO: Usa o novo AppointmentFormSchema que não valida `client_name`.
  ...
  // Função onSubmit totalmente funcional agora
  const onSubmit = async (data: AppointmentFormData) => {
    if (!user) return;
    
    // Encontra o nome do cliente com base no ID selecionado
    ...
    const appointmentData = {
      ...
      client_name: client.name, // Adiciona o nome do cliente aqui
  ...
  const handleSelectEvent = useCallback((event: CalendarEvent) => {
    ...
    reset({
      ...
      // client_name não é necessário aqui, pois não é um campo do form
      ...
      is_confirmed: event.resource.is_confirmed,
    });
    setIsModalOpen(true);
  }, [reset]);
  ...
  const calendarEvents: CalendarEvent[] = appointments.map((appointment: AppointmentType) => {
    ...
    // Adiciona uma duração padrão de 1 hora para o evento no calendário
